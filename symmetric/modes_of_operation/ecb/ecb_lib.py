...
def block_size_gen(length):
    return random.randrange(1, length, 1)

def encrypt(msg, blocksize, key):
    encrypted = ""
    for i in range(math.ceil(len(msg)/blocksize)):
        if (len(msg) >= blocksize):
            encrypted = encrypted + encrypt_bc(msg[0: blocksize], key)
            msg = msg[blocksize: len(msg)]
        else:
            encrypted = encrypted + encrypt_bc(msg, key)
    return encrypted

def decrypt(msg, blocksize, key):
    decrypted = ""
    for i in range(math.ceil(len(msg)/blocksize)):
        if (len(msg) >= blocksize):
            decrypted = decrypted + decrypt_bc(msg[0: blocksize], key)
            msg = msg[blocksize: len(msg)]
        else:
            decrypted = decrypted + decrypt_bc(msg, key)
    return decrypted


def encrypt_bc(msg, key):
    table = [
